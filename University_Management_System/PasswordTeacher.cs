...
{
    public partial class PasswordTeacher : Form
    {
        private SqlConnection connect;
        private string loggedinTid;
        public PasswordTeacher(string getSid)
        {
            InitializeComponent();
        ...
        private void btn_update_Click(object sender, EventArgs e)
        {
            ...
            {
                connect.Open();
                string query = "SELECT Password FROM Teacher WHERE Teacher_ID = @tid";
                SqlCommand cmd = new SqlCommand(query, connect);
                cmd.Parameters.AddWithValue("@tid", loggedinTid);
                object result = cmd.ExecuteScalar();

                if (result != null && result.ToString() == oldPass)
                {
                    string updateQuery = "UPDATE Teacher SET Password = @newPass WHERE Teacher_ID = @tid";
                    SqlCommand updateCmd = new SqlCommand(updateQuery, connect);
                    updateCmd.Parameters.AddWithValue("@newPass", newPass);
                    updateCmd.Parameters.AddWithValue("@tid", loggedinTid);
